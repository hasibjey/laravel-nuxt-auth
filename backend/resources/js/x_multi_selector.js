class xMultiSelector {
    constructor({ containerSelector, title = 'Select item', subTitle = '', placeholder = 'Select items', name = '', itemLimit = Infinity, defaultOption = [] }) {
    ...
    parseDataTargets() {
        const attr = this.container.getAttribute('data-target');
        try {
            return JSON.parse(attr); // Must be like: '["one", "two"]'
        } catch (e) {
            console.error('Invalid data-target format. Use ["one", "two", ...]');
            return [];
        }
    }
    ...
    buildSearchBox() {
        ...
        input.className = 'w-full px-2 py-1 text-sm border-b border-gray-700 outline-none';
    ...
    buildDropdown() {
        ...
        ul.className = 'multiSelector-items text-sm border border-side-secondary rounded-md mt-1 absolute bg-white z-50 w-full hidden max-h-48 overflow-y-auto';
        ...
        this.targets.forEach(item => {
            ...
            li.setAttribute('data-target', item);
    ...
    buildUI() {
        const valueHTML = `
            <label class="block mb-2 font-semibold text-gray-700 capitalize">${this.title} <small class="font-normal text-gray-400">${this.subTitle}</small></label>
            <input type="hidden" class="selected-values" name="${this.name}"/>
            <div class="multiSelector-value w-full border border-side-secondary rounded-md py-2 px-2 text-sm flex flex-wrap gap-1 cursor-default min-h-[40px]">
        ...
        this.input = this.container.querySelector('.selected-values');
    ...
    updateInput() {
        const selected = [...this.valueBox.querySelectorAll('[data-selected]')].map(span => span.getAttribute('data-selected'));
